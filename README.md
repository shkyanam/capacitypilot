...
From **Review queue**, select a customer and complete **Create local reservation**. The application derives the customer region, lets the planner select a compatible QFAB/service/vault reference, and displays the requested TiB against total fresh capacity available in that region. Reservation is disabled when regional inventory is missing, stale, or insufficient. The PostgreSQL transaction locks and rechecks all regional pools before inserting, so concurrent planners cannot over-reserve the regional supply.
